...
$qry="SELECT bname FROM blood_bank_manager where email = '$email'";
$success = $conn->query($qry);
$row = mysqli_fetch_array($success);
$bname = $row[0];
...
$qry = "SELECT hemail FROM ordersblood WHERE oid='$id'";
$success = $conn->query($qry);
$row = mysqli_fetch_array($success);
$hemail = $row[0];
...
$row = mysqli_fetch_array($success);
					...
					if($blood_group == 'A+'){
                        ...
                        {
                   		$query   = "INSERT into previousorders (bemail,name,oid,blood_type,quantity,hemail) VALUES('" . $email . "','" . $bname . "','" . $id . "','" . $blood_group . "','" . $quantity . "','" . $hemail . "')";
						$success = $conn->query($query);
